...
function _strftime($datetime, $format = false)
{
    if ($datetime && strpos($datetime, '0000-00-00') === false) {
        ...
        if (is_numeric($datetime)) {
            ...
            $time = bytic_strftime($datetime);
        }

        if ($time !== false && $time !== -1) {
            return iconv('ISO-8859-2', 'ASCII//TRANSLIT', bytic_strftime($format, $time));
        }
    }
...
if (!function_exists('strftime')) {
    ...
    {
        return bytic_strftime($format, $timestamp, $locale);
    }
}
 ...
 * @internal
 */
function bytic_strftime(string $format, $timestamp = null, ?string $locale = null): string
{
